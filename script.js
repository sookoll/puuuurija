require([], function(cw) {
	...
	function setWorkerEvents() {
		worker.on('json', function(e) {
			m.spin(false);
			var tmp = L.geoJson(e[0], options).addTo(m);
			lc.addOverlay(tmp, e[1]);
			m.fitBounds(tmp.getBounds().pad(0.1));
			setTimeout(function(){
				animateOnAdd(tmp);
			},500);
		});
	...
	function makeDone(div, upButton) {
		...
		doneButton.innerHTML = "Zipitud Shapefile<br>tiri lihtsalt kaardile";
